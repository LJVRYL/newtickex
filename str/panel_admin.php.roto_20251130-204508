...
foreach($colsEv as $c){
    ...
    if ($c['name']==='borrado_en') { $hasBorradoEn=true; }
}

// ===== Detectar evento_id en entradas =====
...
// Si es admin_evento y existe creado_por_admin_id, filtrar por creador
...
// Si existe papelera (borrado_en), mostrar sólo eventos no borrados
if ($hasBorradoEn) {
    $whereEv[] = "(borrado_en IS NULL)";
}
...
<div class="card" style
  <a class="btn secondary" href="crear_evento.php">＋ Crear evento</a>
  ...
  <form method="get" style="margin-top:8px;">
    <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:end;">
      <div style="flex:1 1 260px;">
        <label for="qev">Nombre o slug</label>
        <input
          type="text"
          id="qev"
          name="qev"
    ...
    <table class="table">
      ...
      <?php foreach($eventos as $ev): ?>
        ...
        <tr>
          <td>
            <?php if($flyerOk): ?>
              <img src="<?php echo e($ev['flyer_filename']); ?>" alt="Flyer" style="width:74px;height:74px;object-fit:cover;border-radius:8px;border:1px solid var(--line);background:#000;">
            <?php else: ?>
          ...
          <td>
            <?php
              if($fd==='' && $fh==='') echo '<span style="color:var(--muted);font-size:13px;">Sin fecha</span>';
              else {
                echo e($fd);
                if($fh!=='') echo ' → '.e($fh);
              }
          ...
          <td>
            <div style="display:flex;gap:6px;align-items:center;flex-wrap:nowrap;white-space:nowrap;">
              <a
                class="btn secondary"
                href="panel_evento.php?id=<?php echo $eid; ?>"
                title="Administrar evento"
                style="padding:4px 6px;font-size:12px;"
              >🔍</a>
              <a
                class="btn secondary"
                href="editar_evento.php?id=<?php echo $eid; ?>"
                title="Editar evento"
                style="padding:4px 6px;font-size:12px;"
              >📝</a>
              <a
                ...
                onclick="return confirm('¿Seguro que querés eliminar este evento? Va a la papelera por 30 días.');"
