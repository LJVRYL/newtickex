...
<main class="mp-center">
  ...
  <div class="mp-grid">
    ...
    <section class="card mp-section">
      ...
      <div class="mp-breakdown">
        ...
        <?php if (!$isInternalAccount): ?><div><span>Costo estimado Mercado Pago</span><strong><?php echo e(number_format((float)$settings['mp_cost_estimate_percent'], 2, ',', '.')); ?>%</strong></div><div><span>Fee Tickex estimado</span><strong><?php echo e(number_format((float)$effectiveFee, 2, ',', '.')); ?>%</strong></div><?php endif; ?>
  ...
  <?php if ($isSuper): ?>
    <details class="card mp-super">
      ...
      <div class="mp-super-body"><div class="mp-section-head"><div><h3>Política comercial general</h3><p>Estos valores afectan nuevas órdenes. Los movimientos históricos conservan sus importes.</p></div></div><form method="post"><input type="hidden" name="_csrf" value="<?php echo e($csrf); ?>"><div class="mp-super-grid"><label>Costo de servicio al comprador (%)<input type="number" name="total_cost_target_percent" min="0" max="100" step="0.01" value="<?php echo e($settings['total_cost_target_percent']); ?>"></label><label>Costo estimado Mercado Pago (%)<input type="number" name="mp_cost_estimate_percent" min="0" max="100" step="0.01" value="<?php echo e($settings['mp_cost_estimate_percent']); ?>"></label><div><small class="muted">Fee resultante Tickex</small><strong><?php echo e(number_format((float)$platformFeeGeneral,2,',','.')); ?>%</strong></div></div><label class="mp-toggle"><input type="checkbox" name="enforcement_enabled" value="1"<?php echo $commercialEnabled?' checked':''; ?>> Habilitar ventas Mercado Pago para organizadores cliente</label><button class="btn" type="submit" name="save_platform" value="1">Guardar política general</button></form></div>
    </details>
    <details class="card mp-super">
      <summary>Cuentas y excepciones por administrador · sólo superadmin</summary>
      <div class="mp-super-body"><p class="muted">La cuenta interna debe reservarse para SAVE THE RAVE. El resto permanece como organizador cliente.</p><div class="mp-admin-list"><?php foreach ($adminPolicies as $adminRow): $adminPolicy=$adminRow['mp_policy'];$adminAccount=$adminRow['mp_account']; ?><form method="post" class="mp-admin-policy"><input type="hidden" name="_csrf" value="<?php echo e($csrf); ?>"><input type="hidden" name="policy_admin_id" value="<?php echo (int)$adminRow['id']; ?>"><div><strong><?php echo e(!empty($adminRow['nombre'])?$adminRow['nombre']:'Administrador'); ?></strong><small><?php echo e($adminRow['email']); ?> · MP <?php echo $adminAccount&&$adminAccount['status']==='connected'?'conectado':'sin conectar'; ?></small></div><label>Tipo de cuenta<select name="account_type"><option value="client"<?php echo $adminPolicy['account_type']==='client'?' selected':''; ?>>Organizador cliente</option><option value="str_owner"<?php echo $adminPolicy['account_type']==='str_owner'?' selected':''; ?>>SAVE THE RAVE / interna</option></select></label><label>Costo especial (%)<input type="number" name="fee_override" min="0" max="100" step="0.01" placeholder="Usar política general" value="<?php echo $adminPolicy['platform_fee_override_percent']===null?'':e($adminPolicy['platform_fee_override_percent']); ?>"></label><button class="btn secondary" type="submit" name="save_policy" value="1">Guardar</button></form><?php endforeach; ?></div></div>
    </details>
  <?php endif; ?>
