...
if (!function_exists('tickex_order_batch_email_was_sent')) {
    function tickex_order_batch_email_was_sent($pdo, $orderId)
    {
        $st = $pdo->prepare("SELECT 1 FROM email_logs WHERE related_table = 'tc_orders' AND related_id = :id AND context = 'entradas_compra' AND mail_ok = 1 LIMIT 1");
...
if (!function_exists('tickex_order_send_emails')) {
    function tickex_order_send_emails($pdo, $order, $entries)
    {
        ...
        if (tickex_order_batch_email_was_sent($pdo, $orderId) || tickex_order_legacy_emails_were_sent($pdo, $entries)) {
        ...
        $ticketsText = implode("\n\n", $ticketLines);
        $subject = $quantity === 1 ? 'Tu entrada para el evento' : 'Tus ' . $quantity . ' entradas para el evento';
        $body = "Hola " . $buyerName . ",\n\n";
        ...
        try {
            $mailOk = tickex_send_mail_template(
                ...
                array('context' => 'entradas_compra', 'related_table' => 'tc_orders', 'related_id' => $orderId),
