    ...
    if ($error === '') {
        try {
            ...
            if ($inserted) {
                ...
                @mail($email, $subject, $body, $headers, $extraParams);

                // Log mínimo
                $logLine = date('c') . " reset_mail to=" . $email . " token=" . $token . "\n";
                @file_put_contents(__DIR__ . '/log_mail_reset.txt', $logLine, FILE_APPEND);
            }

            $okMsg = 'Si tu email existe en Tickex, te enviamos un enlace para restablecer la contraseña.';
            $email = '';
            $devLink = $link; // opcional para entornos dev
        } catch (Exception $e) {
...
<?php if ($okMsg !== ''): ?>
  <div class="card" style="max-width:520px;margin:0 auto 12px auto;">
    <div class="flash ok"><?php echo e($okMsg); ?>
      <?php if (!empty($devLink)): ?>
        <div style="margin-top:6px;font-size:12px;">Enlace directo (solo dev): <a class="link" href="<?php echo e($devLink); ?>"><?php echo e($devLink); ?></a></div>
      <?php endif; ?>
    </div>
  </div>
<?php endif; ?>
