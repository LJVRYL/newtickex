  ...
  <style>
    * { box-sizing: border-box; }
    ...
    .event-card {
      ...
      max-width:380px; /* hace que la tarjeta no sea gigante en escritorio */
      margin:0 auto;
    }
    .event-card img {
      width:100%;
      display:block;
      max-height:260px; /* limita el alto en pantallas grandes */
    ...
    @media (max-width: 600px) {
      ...
      .event-card {
        max-width:360px; /* tarjeta más angosta, centrada */
        margin:0 auto;
      }
      .event-card img {
        max-height:none;      /* no recortamos por altura en celu */
        object-fit:contain;   /* respeta formato vertical/cuadrado sin deformar */
      }
    }
  </style>
</head>
        ...
        <?php foreach ($eventos_demo as $ev): ?>
          <article class="event-card">
            <?php if (!empty($ev['flyer']) && file_exists(__DIR__ . '/' . $ev['flyer'])): ?>
              <img src="<?php echo e($ev['flyer']); ?>" alt="<?php echo e($ev['titulo']); ?>">
            <?php endif; ?>
