(function(window, angular, $) {
    ...
    '$scope', '$rootScope', '$translate', '$cookies', '$config', 'item', 'fileNavigator', 'fileUploader', 'dropUploader',
    function($scope, $rootScope, $translate, $cookies, $config, Item, FileNavigator, FileUploader, DropUploader) {
        ...
        $scope.dropUploader = DropUploader;
        ...
        $scope.uploadFiles = function() {
            ...
            $scope.fileUploader.upload($scope.uploadFileList,
                ...
                $scope.temp.error = msg;
            });
        };
        
        $scope.uploadFilesDrop = function() {
            $('#uploading').show();
            $scope.dropUploader.dropupload($scope.dropFileList,
                $scope.fileNavigator.currentPath, function() {
                $scope.fileNavigator.refresh();
                $('#uploaddrop').modal('hide');
                $('#uploading').hide();
            }, function(msg) {
                $scope.temp.error = msg;
            });
        ...
        $scope.$on('eventFired', function(event, data) {
            $('#uploadfile').modal('hide');
            $('#uploaddrop').modal('show');
            $scope.dropFileList = data.fileList;
            $scope.uploadFilesDrop();
        });
